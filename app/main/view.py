...
def add_new_book():
    ...
    if form.validate_on_submit():
        ...
        if exist_id is None:
            ...
            book.book_photo = session['photo']
            session['photo'] = BOOK_PHOTO
            db.session.add(book)
            db.session.commit()
            flash(u'新书入库成功！')
        ...
        if book.book_photo == '':
            ...
            upload_path = os.path.join(static_path, 'books/')
...
def create_order():
    ...
    order.buy_date = str(datetime.now())
    order.local = ''
    # 此时不能加入数据库
    item = {
        # 图书信息
        ...
        if order is None:
            if book is None:
                ...
                if int(want_buy_number) <= int(left_store):
                    ...
                    new_order.buy_date = str(datetime.now())
